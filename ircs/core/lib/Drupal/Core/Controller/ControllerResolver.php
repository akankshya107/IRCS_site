...
class ControllerResolver extends BaseControllerResolver implements ControllerResolverInterface {
  ...
  public function getControllerFromDefinition($controller, $path = '') {
    ...
    if (!is_callable($callable)) {
    	if (is_array($callable) && is_object($callable[0]) && is_string($callable[1])) {
        if ($path) {
          throw new \InvalidArgumentException("Non existing method '{$callable[1]}' on class '\\" . get_class($callable[0]) . "' on path '$path'.");
        }
        throw new \InvalidArgumentException("Non existing method '{$callable[1]}' on class '\\" . get_class($callable[0]) . "'.");
      }
